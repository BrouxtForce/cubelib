...
export class CubeSolver {
    ...
    private cycles: Map<string, number[][]>;
    ...
    calcMoveCycle(moveString: string): void {
        ...
        let searchedPieces: boolean[] = Array(arrayHeight * arrayWidth).fill(false);
        for (let i = 0; i < arrayHeight; i++) {
            for (let j = 0; j < arrayWidth; j++) {
    ...
    private cycle(cycles: number[]): void {
    ...
    solve(depth: number): string[] {
        let moves: Move[] = [];
        for (const key of this.cycles.keys()) {
            moves.push(Move.fromString(key) as Move);
        }
